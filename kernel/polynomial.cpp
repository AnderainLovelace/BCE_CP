...
char* sprint_expression(exp sz) {
	char *sz_new, *temp, *symbol;
	expnode *szptr;
	fact pfx;

	/*  Build a sz_new string  */
	sz_new = (char*) malloc(sizeof(char));
	if (!sz_new)
		return(NULL);
	*sz_new = '\0';
		...
		for (szptr = (expnode*)(sz.stack.ptr); szptr < (expnode*)(sz.stack.ptr) + sz.count; szptr++) {
			...
			if (pfx.numerator % pfx.denominator == 0) {
				if (pfx.numerator / pfx.denominator == 1) {
					kasprintf(&sz_new, "%sX%s+", sz_new, symbol);
				} else {
					if (pfx.numerator / pfx.denominator != 0 && pfx.numerator / pfx.denominator != -1) {
						if (strlen(sz_new) > 0 && pfx.numerator / pfx.denominator < 0)
							if (*(sz_new + strlen(sz_new) - 1) == '+')
								*(sz_new + strlen(sz_new) - 1) = '\0';
						kasprintf(&sz_new, "%s%dX%s+", sz_new, pfx.numerator / pfx.denominator, symbol);
					} else {
						kasprintf(&sz_new, "%s-X%s+", sz_new, symbol);
					}
				}
			} else {
				kasprintf(&sz_new, "%s(%d/%d)X%s+", sz_new, pfx.numerator, pfx.denominator, symbol);
			}
			free(symbol);
			if (sz_new != temp)
				free(temp);
	...
	if (fraction_compare(pfx, F_ZERO) < 0) {
		if (pfx.numerator % pfx.denominator == 0)
			kasprintf(&sz_new, "%s%d", sz_new, pfx.numerator / pfx.denominator);
		else
			kasprintf(&sz_new, "%s%d/%d", sz_new, pfx.numerator, pfx.denominator);
